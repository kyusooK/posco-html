                ...
                <v-list-item v-for="(item, idx) in list" :key="idx">
                    <template v-slot:default="{ active }">
                        ...
                        <v-list-item-content>
                            ...
                            <v-list-item-subtitle>
                                CreatedObjectType :  {{item.createdObjectType }}
                            </v-list-item-subtitle>
                            <v-list-item-subtitle>
                                CreatedObjectId :  {{item.createdObjectId }}
                            </v-list-item-subtitle>
                            <v-list-item-subtitle>
                                CreatedProgramId :  {{item.createdProgramId }}
                            </v-list-item-subtitle>
                            <v-list-item-subtitle>
                                CreationTimestamp :  {{item.creationTimestamp }}
                            </v-list-item-subtitle>
                            <v-list-item-subtitle>
                                LastUpdatedObjectType :  {{item.lastUpdatedObjectType }}
                            </v-list-item-subtitle>
                            <v-list-item-subtitle>
                                LastUpdatedObjectId :  {{item.lastUpdatedObjectId }}
                            </v-list-item-subtitle>
                            <v-list-item-subtitle>
                                LastUpdateProgramId :  {{item.lastUpdateProgramId }}
                            </v-list-item-subtitle>
                            <v-list-item-subtitle>
                                LastUpdateTimestamp :  {{item.lastUpdateTimestamp }}
                            </v-list-item-subtitle>
                            <v-list-item-subtitle>
                                DataEndStatus :  {{item.dataEndStatus }}
                            </v-list-item-subtitle>
                            <v-list-item-subtitle>
                                DataEndObjectType :  {{item.dataEndObjectType }}
                            </v-list-item-subtitle>
                            <v-list-item-subtitle>
                                DataEndObjectId :  {{item.dataEndObjectId }}
                            </v-list-item-subtitle>
                            <v-list-item-subtitle>
                                DataEndProgramId :  {{item.dataEndProgramId }}
                            </v-list-item-subtitle>
                            <v-list-item-subtitle>
                                DataEndTimestamp :  {{item.dataEndTimestamp }}
                            </v-list-item-subtitle>
                            <v-list-item-subtitle>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            if(temp.data) {
                me.list = temp.data._embedded.cars;
            }

            if(me.value && typeof me.value == "object" && Object.values(me.value)[0]) {
                var id = Object.values(me.value)[0];
                var tmpValue = await axios.get(axios.fixUrl('/cars/' + id))
                if(tmpValue.data) {
