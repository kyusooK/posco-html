<template>
    <v-card outlined>
        <v-card-title>
            Car # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <Money offline label="AcquisitionAmt" v-model="item.acquisitionAmt" :editMode="false" @change="change" />
            <div>
            ...
            <DriverIdManager offline label="DriverIds" v-model="item.driverIds" :editMode="false" @change="change" />
            <Usage offline label="Usage" v-model="item.usage" :editMode="false" :key="false" @change="change" />
        </v-card-text>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/cars/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
